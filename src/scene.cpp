...
Scene::Scene(): camera(RATIO), open(true) { 
...
void Scene::load_obj_file( const char * file_name ) {
    ...
    if(file.fail())
        throw std::runtime_error("Object doesn't exist: " + std::string(file_name));
    ...
    while(file.good()) {
        ...
        if(variable == "#")
            ...
            file >> v1 >> v2 >> v3;
            if(cpt%2 == 0)
                listVertex.push_back(Vertex({v1, v2, v3}, 1));
            else
                listVertex.push_back(Vertex({v1, v2, v3}, 0));
            ++cpt;
        }
        else if(variable == "f") {
            std::string s1, s2, s3;
            file >> s1 >> s2 >> s3;
            uint f1 = std::stoul(s1.substr(0, s1.find("/")));
            uint f2 = std::stoul(s2.substr(0, s2.find("/")));
            uint f3 = std::stoul(s3.substr(0, s3.find("/")));
            listFace.push_back(Face(f1-1, f2-1, f3-1, minwin::white));
        }
    }
...
void Scene::initialise() {
    ...
    windows.set_title("Projet Rasterize");
    windows.set_height(HEIGHT);
    windows.set_width(WIDTH);
    windows.load_font("fonts/FreeMonoBold.ttf");
    ...
    windows.register_key_behavior(minwin::KEY_G, new CwXBehavior(*this));
    windows.register_key_behavior(minwin::KEY_T, new AcwXBehavior(*this));
    windows.register_key_behavior(minwin::KEY_H, new CwYBehavior(*this));
    windows.register_key_behavior(minwin::KEY_F, new AcwYBehavior(*this));
    windows.register_key_behavior(minwin::KEY_Y, new CwZBehavior(*this));
    windows.register_key_behavior(minwin::KEY_R, new AcwZBehavior(*this));
...
void Scene::run() {
    ...
    modeText = minwin::Text(10, HEIGHT - 25, "Mode: wireframe", minwin::white);
    minwin::Text spaceText(10, 10, "Espace pour changer de mode", minwin::white);
    while(open) {
...
Vec3r Scene::viewport_to_canvas( const Vec3r & point ) const {
    real cX = ( point[0] * WIDTH ) / 2;
    real cY = ( point[1] * HEIGHT ) / VIEWPORT_HEIGHT;
    real cZ = ( point[2] * (camera.distanceMax - camera.focal_dist) ) / 2;
    return {cX, cY, cZ};
}

Vec3r Scene::canvas_to_window( const Vec3r & point ) const {
    real sX = WIDTH / 2 + point[0];
    real sY = HEIGHT / 2 - point[1];
    real sZ = (camera.distanceMax - camera.focal_dist) / 2 + point[2];
    return {sX, sY, sZ};
}
...
Object Scene::cull_and_clip(const Object& object) {
    Object cull = camera.cull(object);
    return camera.fov.clip(cull);
}

void Scene::draw_object(const Object &object) {
    
    Object obj_transform = apply_object_transform(object);
    Object obj_camera = apply_camera_transform(obj_transform);
    Object obj = cull_and_clip(obj_camera);

    std::vector<Vertex> vertices = obj.get_vertices();
    std::vector<Face> faces = obj.get_faces();

    for(uint j=0; j<faces.size(); ++j) {
        Face f = faces[j];
        windows.set_draw_color(f.color);
        Vertex v1 = vertices[f.v0];
        Vertex v2 = vertices[f.v1];
        Vertex v3 = vertices[f.v2];
        Vec4r p3_1 = {v1.vec[0], v1.vec[1], v1.vec[2], 1};
        Vec4r p3_2 = {v2.vec[0], v2.vec[1], v2.vec[2], 1};
        Vec4r p3_3 = {v3.vec[0], v3.vec[1], v3.vec[2], 1};
...
void Scene::draw_line(const Vec3r &v0, const Vec3r &v1) const {
...
void ForwardXBehavior::on_release() const { owner.getCamera().stop_movement(0); }
...
void BackwardXBehavior::on_release() const { owner.getCamera().stop_movement(0); }
...
void ForwardZBehavior::on_release() const { owner.getCamera().stop_movement(2); }
...
void BackwardZBehavior::on_release() const { owner.getCamera().stop_movement(2); }
...
void ForwardYBehavior::on_release() const { owner.getCamera().stop_movement(1); }
...
void BackwardYBehavior::on_release() const { owner.getCamera().stop_movement(1); }
CwXBehavior::CwXBehavior( Scene & scene ) : owner(scene) {}
void CwXBehavior::on_press() const { owner.getCamera().rotate(0, Clockwise::X); }
void CwXBehavior::on_release() const { owner.getCamera().stop_rotation(0); }
...
void AcwXBehavior::on_release() const { owner.getCamera().stop_rotation(0); }
CwYBehavior::CwYBehavior( Scene & scene ) : owner(scene) {}
void CwYBehavior::on_press() const { owner.getCamera().rotate(1, Clockwise::Y); }
void CwYBehavior::on_release() const { owner.getCamera().stop_rotation(1); }
...
void AcwYBehavior::on_release() const { owner.getCamera().stop_rotation(1); }
CwZBehavior::CwZBehavior( Scene & scene ) : owner(scene) {}
void CwZBehavior::on_press() const { owner.getCamera().rotate(2, Clockwise::Z); }
void CwZBehavior::on_release() const { owner.getCamera().stop_rotation(2); }
...
void AcwZBehavior::on_release() const { owner.getCamera().stop_rotation(2); }