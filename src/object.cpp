...
Object::Object(const Shape &shape, const Vec3r &translation, const Vec3r &rotation, const Vec3r &scale):shape(shape), translation(translation), rotation(rotation), scale(scale) {
        real x = 0, y = 0, z = 0;
        ...
        rayon = 0;
        for(const auto elm: shape.get_vertices()) {
            real d = dist(elm.vec, barycentre);
            if(d > rayon) rayon = d;
        }
    }
